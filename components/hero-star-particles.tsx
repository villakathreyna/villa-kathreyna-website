...
export default function HeroStarParticles() {
  ...
  useEffect(() => {
    ...
    // Create more particles for hero section
    ...
    const animate = () => {
      ...
      particlesRef.current.forEach((particle) => {
        ...
        // Add stronger golden glow for larger particles
        if (particle.size > 1.5) {
          ctx.beginPath()
          ctx.arc(particle.x, particle.y, particle.size * 2.5, 0, Math.PI * 2)
          ctx.fillStyle = `rgba(212, 175, 55, ${particle.opacity * twinkle * 0.3})`
          ctx.fill()
        }
      })
